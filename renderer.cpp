...
void Renderer::MouseProcessing(int button)
{
	...
	{	
		if (scn->picked) {
			...
			if (scn->selected_data_index <= scn->last_link_index && scn->selected_data_index >= scn->root_link_index) {
				scn->data_list[scn->root_link_index].MyTranslate(Eigen::Vector3f(-xrel / 1000.0f, 0, 0), true);
				scn->data_list[scn->root_link_index].MyTranslate(Eigen::Vector3f(0, yrel / 1000.0f, 0), true);
			}
			else {
				//scn->data().MyTranslate(Eigen::Vector3f(-xrel / 500.0, 0, 0), false);
				//scn->data().MyTranslate(Eigen::Vector3f(0, yrel/500.0 , 0), false);
				scn->data().TranslateInSystem(scn->MakeTrans() ,Eigen::Vector3f(-xrel / 250.0, 0, 0), true);
				scn->data().TranslateInSystem(scn->MakeTrans(), Eigen::Vector3f(0, yrel / 250.0, 0), true);
			}
		}
